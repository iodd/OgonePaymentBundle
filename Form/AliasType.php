    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->addValidator(
            new CallbackValidator(
                function (FormInterface $form) use ($translator, $transactionManager) {
                    if ($form->get('date_year')->getData() . $form->get('date_month')->getData() < date('ym')) {
                        $form->get('date_month')->addError(
                            new FormError($translator->trans("alias.date_expired", array(), 'ogone'))
                        );
                    ...
                    if (mb_strlen($form->get('card_cvv')->getData()) != 3) {
                        $form->get('card_cvv')->addError(
                            new FormError($translator->trans("card_cvv.invalid", array(), 'ogone'))
                        );
